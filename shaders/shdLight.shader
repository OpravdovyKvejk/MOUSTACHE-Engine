...
uniform vec3 fogrange;//z1,z2,enable
...
void main()
{
    ...
    fog = clamp(((gm_Matrices[MATRIX_WORLD_VIEW] * object_space_pos).z-fogrange.x)/abs(fogrange.y-fogrange.x),0.0,ceil(fogrange.z));
}
...
void main()
{
    ...
    gl_FragColor = vec4(mix(total+Col.rgb*ambience,fogcolor,clamp(fog,0.0,1.0)),Col.a);
}
